...
function transformNode(node, srcFilePath, path, pkgName, withTypes, noServerImport, noDecorator) {
    if (node.type === 'ExportNamedDeclaration') {
        if (!node.declaration) {
            ...
            return transformFunc_1.transformFunc(node.declaration, srcFilePath, path, pkgName, withTypes);
        }
        else if (node.declaration.type === 'VariableDeclaration') {
            return transformArrowFunc_1.transformArrowFunc(node.declaration, srcFilePath, path, pkgName, withTypes);
        }
        else if (node.declaration.type === 'ClassDeclaration') {
            return transformClass_1.transformClass(node, srcFilePath, path, pkgName, withTypes, noDecorator);
        }
    }
