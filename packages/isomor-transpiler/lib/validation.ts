...
export function setValidator(
    ...
    if (args.length !== paramRoot.params.length) {
        warn(
            'Validatormight not recognize one of your params type. \
            Please report the warning at https://github.com/apiel/isomor/issues',
            srcFilePath,
            name,
        );
    ...
    if (!process && queueList.length) {
        ...
        process = exec(command, async (err, stdout, stderr) => {
            ...
            if (stdout && stdout.length) {
                ...
                info(
                    `JSON schema generation finished for ${name} in ${srcFilePath}`,
                );
            } else {
                warn(
                    `JSON schema generation empty for ${name} in ${srcFilePath}`,
                );
            }
            process = null;
            run();
