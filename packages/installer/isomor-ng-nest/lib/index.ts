...
async function start({ srcFolder, distAppFolder, serverFolder }: Options) {
    try {
        ...
        if (!projectDirectory) {
            warn(
                `Please provide the project name, e.g: npx isomor-ng-nest my-app`,
            );
            return;
        }
        await shell(npx, [
        ...
        copySync(
            join(__dirname, '..', 'example', 'server'),
            join(projectDirectory, srcFolder, 'server'),
        );
        copySync(
            join(__dirname, '..', 'example', 'uptime'),
            join(projectDirectory, srcFolder, 'app', 'uptime'),
        );
        let AppModule = readFileSync(
            join(projectDirectory, srcFolder, 'app', 'app.module.ts'),
        ).toString();
        AppModule =
            `
...
import { UptimeComponent } from './uptime/uptime.component';
        ...
        let AppHtml = readFileSync(
            join(projectDirectory, srcFolder, 'app', 'app.component.html'),
        ).toString();
        ...
        info('Setup proxy');
        copySync(
            join(__dirname, '..', 'proxy.conf.json'),
            join(projectDirectory, 'proxy.conf.json'),
        );
        const angularJson = readJSONSync(
            join(projectDirectory, 'angular.json'),
        );
