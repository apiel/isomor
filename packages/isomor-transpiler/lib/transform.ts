...
export default function transform(body: TSESTree.Statement[], fileName: string, withTypes: boolean = true) {
    body.forEach((node, index) => {
        if (node.type === 'ExportNamedDeclaration') {
            if (node.declaration.type === 'TSInterfaceDeclaration') {
                ...
                body[index] = getCodeFunc(fileName, name, withTypes);
                ...
                if (declaration.type === 'VariableDeclarator'
                    ...
                    body[index] = getCodeArrowFunc(fileName, name, withTypes);
                } else {
